...
if [ -d .git ]; then
  ...
  if ! git diff --quiet || ! git diff --cached --quiet; then
    ...
    echo "local changes present; auto-stashing (${stash_tag}) before updating to ${update_target}"
    git stash push --include-untracked -m "$stash_tag" >/dev/null
    stashed=1
  fi

  echo "updating code: git fetch origin main"
  git fetch origin main
  echo "updating code: git merge --ff-only ${update_target} (current branch: ${branch})"
  if ! git merge --ff-only "$update_target"; then
    echo "ERROR: cannot fast-forward ${branch} to ${update_target}; refusing to run stale code." >&2
    if [ "$stashed" -eq 1 ]; then
      echo "Your local edits are saved in the latest run.sh-autostash entry. Inspect with: git stash list" >&2
    fi
    exit 1
  fi
  ...
  if [ "$stashed" -eq 1 ]; then
    ...
    if ! git stash pop >/dev/null 2>&1; then
      echo "ERROR: 'git stash pop' had conflicts after updating code; refusing to run." >&2
      echo "Your local edits are still saved in the stash. Inspect with: git stash list" >&2
      exit 1
    fi
  fi

  # ALWAYS print HEAD so the log makes it unambiguous which code ran.
  echo "=== RUNNING COMMIT $(git rev-parse --short HEAD) ==="
  git log -1 --pretty=format:'  %h %s%n  author: %an  date: %ai%n' HEAD
  echo
fi
...
run_variant_pass() {
  ...
  # Variant-metadata wiring. The CLI flag takes a TSV with class-aware
  # annotations. The SV VCFs already supply per-record length / is_copy_number /
  # is_repeat / quality automatically — this is for *additional* annotations
  # (LOEUF, ClinGen, regulatory-element overlaps, etc.) the user has authored.
  # Priority: $VARIANT_METADATA env override → repo-local file → per-disease file.
  local metadata_args=()
  local md_candidates=(
    "${VARIANT_METADATA:-}"
    "$REPO_DIR/variant_metadata.tsv"
    "$HOME/sv_pgs_metadata/variants.tsv"
    "$HOME/sv_pgs_metadata/${DISEASE:-all}.variants.tsv"
  )
  for md in "${md_candidates[@]}"; do
    if [ -n "$md" ] && [ -f "$md" ]; then
      metadata_args=(--variant-metadata "$md")
      echo "  variant metadata: $md"
      break
    fi
  done
  if [ ${#metadata_args[@]} -eq 0 ]; then
    echo "  variant metadata: none supplied (set VARIANT_METADATA=/path/to/file.tsv to enable; SV VCFs still contribute SVTYPE/SVLEN/repeat fields automatically)"
  fi
  ...
  if is_all; then
    ...
    env -i "${env_args[@]}" LAST_OUTDIR="$base" \
      uv run sv-pgs run-all-of-us --all-diseases --variants "$variants" --output-dir "$base" "${metadata_args[@]}"
    rc=$?
  else
    ...
    env -i "${env_args[@]}" LAST_OUTDIR="$out" \
      uv run sv-pgs run-all-of-us --disease "$DISEASE" --variants "$variants" --output-dir "$out" "${metadata_args[@]}"
    rc=$?
  fi
  set -e
  if [ "$rc" -ne 0 ]; then
...
# SNP+SV first: SV calls carry per-record SVTYPE/SVLEN/is_copy_number/
# is_repeat into VariantRecord automatically, which feeds the class-aware
# prior. SNP-only is the marginal/baseline.
