...
if [ -d .git ]; then
  ...
  # uv.lock is regenerated by ``uv sync`` immediately below; checking it out
  # before pull lets ff-only succeed without ever blocking on the lockfile
  # churn. Other dirty paths still need stash + pop.
  if ! git diff --quiet -- uv.lock 2>/dev/null; then
    echo "  uv.lock dirty — discarding (uv sync will regenerate it)"
