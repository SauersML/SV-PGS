...
def signed_code_moments(
    ...
    selected_sums = np.empty(stop - start, dtype=np.int64)
    selected_squares = np.empty(stop - start, dtype=np.int64)
    ...
    for block_start, block_stop, block in store.iter_codes(ranges, None, budget):
        ...
        all_sums, all_squares = code_sums(block)
        ...
        gathered_sums, gathered_squares = code_sums(block[:, gathered])
        selected_sums[local] = all_sums - gathered_sums if use_complement else gathered_sums
        selected_squares[local] = all_squares - gathered_squares if use_complement else gathered_squares
    count = int(selection.indices.size)
    return SignedCodeMoments(
        sample_count=count,
        signed_sums=selected_sums - SIGNED_CODE_OFFSET * count,
        signed_square_sums=selected_squares - 2 * SIGNED_CODE_OFFSET * selected_sums + _MAXIMUM_SIGNED_PRODUCT * count,
    )
