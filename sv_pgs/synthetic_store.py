...
def _lowest_set_bit(masks: NDArray) -> I64Array:
    bits = np.unpackbits(masks.astype("<u8").view(np.uint8).reshape(-1, 8), axis=1, bitorder="little")
...
def lay_out_chromosome(
    ...
    (its nested record becomes a single-path record of its first carried path), so every shard is
    ...
    while produced < record_count:
        ...
        source_index = unit_starts[unit_of_record] + np.where(nested, 0, offset)
        nested_rows = np.flatnonzero(nested)
        source_index[nested_rows] = unit_starts[unit_of_record[nested_rows]] + _lowest_set_bit(masks[nested_rows])
        pieces.append(
            {
                "kind": kind,
...
def haplotype_posteriors(
    ...
    ``parameters`` hold one row per truth row; ``ancestry_frequency`` is [rows, ancestries].
    """
    draw = rng.random(truth.shape, dtype=np.float32)
    carrier = truth.astype(bool)
    uninformed = parameters.uninformed[:, None]
    flip_upper = uninformed + np.where(carrier, parameters.carrier_flip[:, None], parameters.noncarrier_flip[:, None])
    posterior = truth.astype(np.float32)
    flipped = (draw >= uninformed) & (draw < flip_upper)
    posterior[flipped] = 1.0 - posterior[flipped]
    soft = (draw >= flip_upper) & (draw < flip_upper + parameters.soft[:, None])
    soft_rows, soft_columns = np.nonzero(soft)
    fraction = (draw[soft_rows, soft_columns] - flip_upper[soft_rows, soft_columns]) / parameters.soft[soft_rows]
    delta = np.power(
        np.float32(10.0),
        SOFT_DELTA_LOG10_RANGE[0] + (SOFT_DELTA_LOG10_RANGE[1] - SOFT_DELTA_LOG10_RANGE[0]) * fraction,
    )
    posterior[soft_rows, soft_columns] = np.abs(posterior[soft_rows, soft_columns] - delta)
    uninformed_rows, uninformed_columns = np.nonzero(draw < uninformed)
    posterior[uninformed_rows, uninformed_columns] = ancestry_frequency[
        uninformed_rows, haplotype_ancestry[uninformed_columns]
    ]
...
def _row_modes(values: NDArray, bins: int) -> tuple[NDArray, NDArray]:
    modes = np.empty(values.shape[0], dtype=np.int64)
    counts = np.empty(values.shape[0], dtype=np.int64)
    for row, row_values in enumerate(values):
        histogram = np.bincount(row_values, minlength=bins)
        modes[row] = int(np.argmax(histogram))
        counts[row] = int(histogram[modes[row]])
    return modes, counts
...
def half_statistics(haplotype_probabilities: F32Array) -> tuple[U8Array, dict[str, NDArray]]:
    ...
    dosage_milli = np.rint((first + second) * 1000.0).astype(np.int64)
    np.clip(dosage_milli, 0, MAXIMUM_DOSAGE_MILLI, out=dosage_milli)
    codes = encode_dosage_milli(dosage_milli)
    variance_micro = np.rint((first * (1.0 - first) + second * (1.0 - second)) * 1e6).astype(np.int64)
    alt_calls = (first > 0.5).astype(np.int8) + (second > 0.5).astype(np.int8)
    ds_modes, ds_mode_counts = _row_modes(dosage_milli, MAXIMUM_DOSAGE_MILLI + 1)
    code_modes, code_mode_counts = _row_modes(codes, 256)
    wide_codes = codes.astype(np.int64)
    sample_count = codes.shape[1]
    statistics = {
        "sum_ds": dosage_milli.sum(axis=1),
        "sum_ds2": (dosage_milli * dosage_milli).sum(axis=1),
        "sum_var": variance_micro.sum(axis=1),
        "n_het": (alt_calls == 1).sum(axis=1),
        "n_homalt": (alt_calls == 2).sum(axis=1),
        "n_ge05": (dosage_milli >= 500).sum(axis=1),
        ...
        "n_off_mode_code": sample_count - code_mode_counts,
        "sum_code": wide_codes.sum(axis=1),
        "sum_code2": (wide_codes * wide_codes).sum(axis=1),
    }
