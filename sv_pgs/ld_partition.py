...
def fixed_point_pair_weights(
    ...
    operation on exact integers; ``genotype_buffers``' CUDA kernel takes the same steps, so both
    give the same bits.
    """
    numerator = profile_count * band.astype(np.int64) - np.outer(row_sums, column_sums)
    ...
    denominator = np.where(row_varies, row_variance, 1.0)[:, None] * np.where(column_varies, column_variance, 1.0)[None, :]
    ...
    distance = first_distance + np.arange(band.shape[1], dtype=np.int64)[None, :] - np.arange(band.shape[0], dtype=np.int64)[:, None]
...
class LdBlockBoundaries:
    """The data-driven LD blocks of one genotype pass, in store order.

    Block ``b`` holds the pass's candidate rows in store rows ``[block_starts[b], block_stops[b])``.
    ``cut_costs[chromosome]`` is ``C(k)`` over that chromosome's streamed variants (fixed
    point), the LD profile the boundaries minimize.
    """

    block_cap: int
    chromosomes: tuple[str, ...]
    block_chromosomes: NDArray[np.int32]
    block_starts: NDArray[np.int64]
    block_stops: NDArray[np.int64]
    cut_costs: dict[str, NDArray[np.int64]]
    forced_cuts: int
    ...
    def signature_sha256(self) -> str:
        """Digest of the cap and every block, for cache keys downstream."""
        digest = hashlib.sha256()
        digest.update(np.int64(self.block_cap).tobytes())
        digest.update("\0".join(self.chromosomes).encode("utf-8"))
        for values in (self.block_chromosomes, self.block_starts, self.block_stops):
