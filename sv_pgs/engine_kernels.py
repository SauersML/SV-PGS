...
# The host twin of ``objective_rows``: the same per-node terms and running log-sum-exp in compiled loops, writing D and
# G row-major so their Grams are one BLAS call each. The host path's objective was a chain of p x K numpy passes (the
# kernel rows' GEMV, the responsibilities, their products): 83 ms a call, 622 calls a bench-real fit [real,
# ENSG00000138468.16, p = 24,605, K = 40, one core].
@numba.njit(cache=True, inline="always")
def _host_node_terms(log_scale_value, scale_exp, node, node_exp, precision, shift_square, tiny, huge):
    variance = scale_exp * node_exp
    if not (scale_exp >= tiny and scale_exp <= huge and node_exp >= tiny and node_exp <= huge and variance >= tiny and variance <= huge):
        variance = math.exp(log_scale_value + node)
    ...
    if abs(ratio) * tiny < 0.5:
        ...
        conditional = 1.0 / (1.0 / variance + precision)
    signal = shift_square * conditional
    if math.isinf(variance) and precision > 0.0:
        log_root = -0.5 * (log_scale_value + node + math.log(precision))
    else:
        log_root = -0.5 * math.log1p(ratio)
    return retained, ratio_retained, signal, log_root, bad


@numba.njit(cache=True)
def _host_objective_rows(class_log_density, class_density, nodes, node_exp, log_scale, precision, shift, deviations, centred,
                         log_normalizer, curvature, mean_first):
    tiny = np.finfo(np.float64).tiny
    huge = np.finfo(np.float64).max
    node_count = nodes.shape[0]
    improper = False
    for row in range(log_scale.shape[0]):
        scale_value = log_scale[row]
        scale_exp = math.exp(scale_value)
        cavity_precision = precision[row]
        ...
        for node in range(node_count):
            retained, ratio_retained, signal, log_root, bad = _host_node_terms(
                scale_value, scale_exp, nodes[node], node_exp[node], cavity_precision, shift_square, tiny, huge)
            improper = improper or bad
            exponent = class_log_density[node] + 0.5 * signal + log_root
            ...
            if math.isinf(exponent):
                continue
            if exponent > peak:
                rescale = math.exp(peak - exponent)
                total *= rescale
                first_square *= rescale
                peak = exponent
                weight = 1.0
            else:
                weight = math.exp(exponent - peak)
        ...
        for node in range(node_count):
            ...
            exponent = class_log_density[node] + 0.5 * signal + log_root
            first = 0.5 * (retained * signal - ratio_retained)
            responsibility = 0.0 if math.isinf(exponent) else math.exp(exponent - peak) * inverse_total
            deviations[row, node] = responsibility - class_density[node]
            centred[row, node] = responsibility * (first - first_mean)
        log_normalizer[row] = peak + math.log(total)
        curvature[row] = first_square / total + second_mean
        mean_first[row] = first_mean
    return improper


def objective_statistics_host(
    class_rows: Sequence[np.ndarray],
    log_density: np.ndarray,
    log_scale_rows: np.ndarray,
    grid: np.ndarray,
    ...
    scale_design: np.ndarray,
    working_bytes: int,
    hessian_too: bool = True,
) -> tuple[float, np.ndarray, np.ndarray, float]:
    """``objective_statistics`` on the host, by ``_host_objective_rows`` and BLAS Grams over chunks of rows sized from
    ``working_bytes`` (two row x K float64 buffers); with ``hessian_too`` false the Hessian's Grams are skipped."""
    if working_bytes <= 0:
        raise ValueError("working_bytes must be positive")
    class_count, node_count = log_density.shape
    scale_size = int(scale_design.shape[1])
    scale_span = slice(class_count * node_count, class_count * node_count + scale_size)
    dimension = class_count * node_count + scale_size
    nodes = np.ascontiguousarray(grid, dtype=np.float64)
    node_exp = np.exp(nodes)
    density = np.exp(log_density)
    chunk = max(1, int(working_bytes // (2 * 8 * (node_count + scale_size + 2))))
    gradient = np.zeros(dimension)
    hessian = np.zeros((dimension, dimension))
    value = magnitude = 0.0
    improper = False
    for class_position, all_rows in enumerate(class_rows):
        rows_all = np.asarray(all_rows)
        span = slice(class_position * node_count, (class_position + 1) * node_count)
        class_log_density = np.ascontiguousarray(log_density[class_position], dtype=np.float64)
        class_density = np.ascontiguousarray(density[class_position], dtype=np.float64)
        summed = np.zeros(node_count)
        outer = np.zeros((node_count, node_count))
        cross = np.zeros((node_count, scale_size))
        for start in range(0, int(rows_all.shape[0]), chunk):
            rows = rows_all[start : start + chunk]
            count = int(rows.shape[0])
            deviations = np.empty((count, node_count))
            centred = np.empty((count, node_count))
            log_normalizer = np.empty(count)
            curvature = np.empty(count)
            mean_first = np.empty(count)
            improper |= _host_objective_rows(
                class_log_density, class_density, nodes, node_exp,
                np.ascontiguousarray(log_scale_rows[rows], dtype=np.float64), np.ascontiguousarray(precision[rows], dtype=np.float64),
                np.ascontiguousarray(shift[rows], dtype=np.float64), deviations, centred, log_normalizer, curvature, mean_first,
            )
            value += float(log_normalizer.sum())
            magnitude += float(np.abs(log_normalizer).sum())
            summed += deviations.sum(axis=0)
            if scale_size:
                design = np.asarray(scale_design[rows], dtype=np.float64)
                # the scale gradient sum_j (mean d1)_j s_j (the device's extra row under G)
                gradient[scale_span] += design.T @ mean_first
            if hessian_too:
                outer += deviations.T @ deviations
                if scale_size:
                    cross += centred.T @ design
    ...
    _raise_if_improper(np.array([float(improper)]))
    return value, gradient, hessian, magnitude
