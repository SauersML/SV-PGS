...
# Each block row costs one byte per sample in each of the store's (at most three)
# read-ahead buffers plus eight in the fp64 conversion panels; a block uses half
# the host budget.
_HOST_BYTES_PER_BLOCK_CELL = 3 + 8
...
class CodeReader(Protocol):
    """The part of ``dosage_store.DosageStore`` scoring reads through."""
    ...
    def iter_codes(
        ...
        """Yield (start, stop, codes[stop - start, selected samples]) for each range, in order."""
    ...
    def read_codes(
        ...
        """Codes [stop - start, selected samples] of one row range."""
...
def _read_plan_blocks(
    ...
    row_ranges = [(store_start, store_stop) for store_start, store_stop, _ in ranges]
    for store_start, store_stop, codes in reader.iter_codes(row_ranges, sample_indices, budget):
        yield store_start, store_stop, plan_starts[(store_start, store_stop)], codes


def _selected_sample_count(reader: CodeReader, sample_indices: NDArray | None) -> int:
    return reader.n_samples if sample_indices is None else int(np.asarray(sample_indices).shape[0])
...
def score_genetic(
    ...
    ``sample_indices`` (sorted store columns) selects samples, all of them when ``None``;
    scoring all samples once and slicing folds afterwards reads the store once for every
    fold.
    """
    sample_count = _selected_sample_count(reader, sample_indices)
    block_rows = _block_rows(sample_count, budget, int(plan.store_rows.shape[0]))
    ...
    blocks = _read_plan_blocks(reader, ranges, sample_indices, budget)
    if budget.device_kind == "cuda":
...
def block_predictor_variance(
    ...
    it is meant for small problems and for the samples whose predictive probabilities
    are needed.
    """
    variance = np.zeros(_selected_sample_count(reader, sample_indices), dtype=np.float64)
    for block in blocks:
        row_start, row_stop = int(block.store_rows[0]), int(block.store_rows[-1]) + 1
        codes = reader.read_codes(row_start, row_stop, sample_indices)[block.store_rows - row_start]
        standardized = (
            np.subtract(codes, SIGNED_CODE_OFFSET, dtype=np.float64) - block.signed_means[:, None]
