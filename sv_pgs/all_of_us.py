...
def _occasion_design(ages: np.ndarray, female: np.ndarray, sex_levels: list[str]) -> np.ndarray:
    ...
    level other than the most common one (a missing value is its own level) and centered age x female. A column
    that adds no rank to the ones before it (one sex only, or too few distinct ages) is left out."""
    ...
    for column in columns:
        if np.linalg.matrix_rank(np.column_stack([*kept, column])) > len(kept):
            kept.append(column)
