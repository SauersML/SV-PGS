...
# Refresh the posterior variance every other iteration; iterations between
# refreshes carry the last posterior diagonal re-aligned to the current prior.
# Final posterior diagnostics stay off for the memory budget.
AOU_BETA_VARIANCE_UPDATE_INTERVAL = 2
...
def _describe_phenotype_source(phenotype_definition: DiseaseDefinition | MeasurementDefinition) -> str:
    ...
    return (
        f"condition SNOMED roots {', '.join(phenotype_definition.case_snomed_codes)} "
        f"({phenotype_definition.description})"
    )
...
def run_all_of_us(
    ...
    # dir so single-disease invocations (e.g. `./run.sh type2_diabetes`)
