...
def tilted_moments(
    ...
    if array_module is not np:
        on_device = engine_kernels.tilted_moments(
            ...
            log_scale(prior, hyperparameters.coefficients), prior.log_variance_grid, prior.kernel_floor,
            cavity.precision, cavity.shift, working_bytes,
        )
...
def _data_objective(
    ...
    if array_module is not np:
        value, gradient, hessian, magnitude = engine_kernels.objective_statistics(
            ...
            prior.log_variance_grid, prior.kernel_floor, cavity.precision, cavity.shift, prior.scale_design, working_bytes,
        )
...
def _data_value(
    ...
    if array_module is not np:
        log_normalizer, _mean, _variance = engine_kernels.tilted_moments(
            array_module, prior.class_index, log_density, scales, prior.log_variance_grid, prior.kernel_floor,
            cavity.precision, cavity.shift, working_bytes,
        )
