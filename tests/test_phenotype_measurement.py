...
def test_the_modulus_bound_covers_the_mass_weighted_modulus(residuals):
    prior = _lattice(1.0 / 12.0, 5000.0, 12, 2)
    hyperparameters = _density(prior, {_node(prior, 1.0): 0.9, _node(prior, 1.0 / 12.0): 0.05, _node(prior, 2500.0): 0.05})
    masses = np.exp(class_log_density(prior, hyperparameters.coefficients)[0])
    variances = np.exp(prior.log_variance_grid)
    level_variance, residual_array = 4.0, np.array([residuals])
    for step, half_width in ((0.05, 0.3), (0.4, 0.05), (1.0, 1.0)):
        # Nodes over part of the line only, so the two tail bounds carry the rest.
        levels = np.arange(-3.0, 6.0 + step, step)[None, :]
        valid = np.ones_like(levels, dtype=bool)
        bound = _log_modulus_bound(residual_array, levels, valid, np.array([step]), level_variance, np.log(masses), variances,
...
def test_louis_information_is_the_curvature_of_the_exact_log_likelihood():
    ...
    def log_likelihood(coefficients: np.ndarray) -> float:
        ...
        return float(level_posterior(residuals, 4.0, log_masses, variances, tolerance, fixed, False, WORKING_BYTES).log_likelihood.sum())
    ...
    first = level_posterior(residuals, 4.0, log_masses, variances, tolerance, None, False, WORKING_BYTES)
    grid = first.grid
    # Twice the reaches, so no perturbed density grows a grid either.
    fixed = LevelGrid(0.5 * first.admissible_step, first.level_mean, 2.0 * grid.lower_reach, 2.0 * grid.upper_reach, grid.half_width)
    posterior = level_posterior(residuals, 4.0, log_masses, variances, tolerance, fixed, True, WORKING_BYTES)
...
def test_gross_errors_are_downweighted_by_the_learned_density():
    noise = 1.0
    person_index, values, gross_values, design, gross = _simulated(300, 4.0, noise, seed=4, gross_share=0.04)
    ...
    # At the fit's own parameters, each person's level without their gross readings (at the prior when none is
    # left) ...
    persons = person_index.max() + 1
    kept = ~gross
    remaining = np.unique(person_index[kept])
    without_mean, without_sd = np.zeros(persons), np.full(persons, np.sqrt(fit.level_variance))
    without = level_posterior_at(
        Occasions(person_index=np.searchsorted(remaining, person_index[kept]), values=values[kept], design=design[kept]),
        1.0, fit.fixed_effects, fit.level_variance, fit.prior, fit.hyperparameters, WORKING_BYTES,
    )
    without_mean[remaining], without_sd[remaining] = without.level_mean, np.sqrt(without.level_posterior_variance)
    # ... is within one posterior standard deviation of their level with them: the learned density downweights them.
    np.testing.assert_array_less(np.abs(fit.level_mean[affected] - without_mean[affected]), without_sd[affected])
    # Gaussian noise of the simulated variance (the dense Henderson solution) moves every one of them by more.
    residuals = gross_values - design @ fit.fixed_effects
    counts = np.bincount(person_index, minlength=persons)
    gaussian_mean = np.bincount(person_index, weights=residuals, minlength=persons) / noise / (1.0 / fit.level_variance + counts / noise)
    np.testing.assert_array_less(without_sd[affected], np.abs(gaussian_mean[affected] - without_mean[affected]))
