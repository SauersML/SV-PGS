...
from sv_pgs.phenotype_measurement import Occasions
...
def test_occasion_design_models_age_by_sex_and_drops_empty_columns():
    ages = np.array([40.0, 50.0, 60.0, 70.0])
    female = np.array([1.0, 0.0, 1.0, 0.0])
    design = _occasion_design(ages, female, ["45878463", "45880669", "45878463", "45880669"])
    ...
    assert _occasion_design(ages, np.zeros(4), ["45880669"] * 4).shape == (4, 3)


def test_the_model_target_tracks_the_true_level_better_than_the_raw_mean():
    rows, levels = _synthetic_person_rows(400, level_variance=1.0, noise_variance=2.0, seed=7)
