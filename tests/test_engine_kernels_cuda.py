...
def _class_bounds(log_density, scales, grid, floor, class_rows, precision, shift):
    ...
    A node of log weight -inf is dropped exactly on both sides (an overflowing v), so its errors are zero.
    """
    u, t = _UNIT, _TRANSCENDENTAL
    node_count = grid.shape[0]
    active = grid[None, :] >= floor
    for class_position, rows in enumerate(class_rows):
        arguments = (log_density[class_position], scales[rows], grid, floor, precision[rows], shift[rows])
        ...
        with np.errstate(over="ignore", invalid="ignore", divide="ignore"):
            ...
            variance = np.where(active, np.exp(exponent), 0.0)
            ...
            v_error = np.where(active, u * np.abs(exponent) + 2.0 * t + u + spacing, 0.0)
            q_error = v_error + u
            ...
            c_lost = np.where(active & (variance * _LARGEST < 1.0), variance * r, 0.0)
            qr_lost = np.where(active & (q * _LARGEST < 1.0), q * r, 0.0)
...
def _assert_tilted_parity(log_density, scales, grid, floor, class_rows, precision, shift, device, host) -> None:
    ...
    u = _UNIT
    node_count = grid.shape[0]
    for _class, rows, node in _class_bounds(log_density, scales, grid, floor, class_rows, precision, shift):
...
def _assert_tilted_moments_match(prior, hyperparameters, cavity, working_bytes=_WORKING_BYTES) -> None:
    ...
    _assert_tilted_parity(
        ...
        prior.kernel_floor, prior.class_rows, cavity.precision, cavity.shift,
...
def _assert_objective_matches(prior, coefficients, cavity, working_bytes) -> None:
    ...
    for class_position, rows, node in _class_bounds(
        class_log_density(prior, coefficients), log_scale(prior, coefficients), prior.log_variance_grid, prior.kernel_floor,
...
def test_cuda_underflowing_and_overflowing_variances_take_the_hosts_values():
    ...
    terms = _components(log_density[0], scales, grid, floor, precision, shift)
    ...
    device = engine_kernels.tilted_moments(cupy, class_index, log_density, scales, grid, floor, precision, shift, _WORKING_BYTES)
    _assert_tilted_parity(
        log_density, scales, grid, floor, [everything], precision, shift,
