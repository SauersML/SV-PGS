...
REGISTRY: dict[tuple[str, str], tuple[str, frozenset[object], str]] = {
    ...
    ('scale_mixture_ep.py', '_maximize_coefficients'): ('spec', frozenset({0.25, 0.75}), 'Nocedal and Wright (2006), Numerical Optimization, Algorithm 4.1: the trust-region radius update'),
    ('scale_mixture_ep.py', '_ascend_evidence'): ('spec', frozenset({0.25, 0.75}), 'Nocedal and Wright (2006), Numerical Optimization, Algorithm 4.1: the trust-region radius update'),
    ...
    ('scale_mixture_ep.py', '_stationarity_check'): ('derived', frozenset({3.0, 6.0, 4.0, 0.75, 0.25}), 'central difference with V certified to e: error h^2 s/6 + e/h, least at h = (3e/s)^(1/3) where it is (3^(2/3)/2) s^(1/3) e^(2/3); e from 1/2 E^2/s = tolerance/(4n), i.e. e = (2 tolerance/(n 3^(4/3)))^(3/4) s^(1/4)'),
