...
REGISTRY: dict[tuple[str, str], tuple[str, frozenset[object], str]] = {
    ...
    ('code_products.py', 'CodeBlockTile._codes_times'): ('derived', frozenset({4}), "byte accounting of this module's arrays: itemsizes and the number of simultaneously live arrays"),
    ('code_products.py', 'CodeBlockTile._codes_times_block'): ('derived', frozenset({3}), "byte accounting of this module's arrays: itemsizes and the number of simultaneously live arrays"),
    ('code_products.py', 'CodeBlockTile._digit_products'): ('derived', frozenset({4}), "byte accounting of this module's arrays: itemsizes and the number of simultaneously live arrays"),
    ...
    ('draw_laws.py', '_PHILOX_ROUNDS'): ('spec', frozenset({10}), 'Philox4x32-10: the ten rounds Salmon et al. 2011 recommend (Crush-resistant with margin)'),
    ('draw_laws.py', '_WORD_BITS'): ('definitional', frozenset({32}), 'Philox4x32 words are 32 bits'),
    ('draw_laws.py', '_WORD_MASK'): ('definitional', frozenset({0xFFFFFFFF}), 'the low 32 bits of a uint64'),
    ('draw_laws.py', '_HIGH_SHIFT'): ('spec', frozenset({5}), 'genrand_res53 (Matsumoto and Nishimura, MT19937 reference): (a >> 5) 2^26 + (b >> 6), over 2^53'),
    ('draw_laws.py', '_LOW_SHIFT'): ('spec', frozenset({6}), 'genrand_res53 (Matsumoto and Nishimura, MT19937 reference): (a >> 5) 2^26 + (b >> 6), over 2^53'),
    ('draw_laws.py', '_LOW_BITS'): ('spec', frozenset({26}), 'genrand_res53 (Matsumoto and Nishimura, MT19937 reference): (a >> 5) 2^26 + (b >> 6), over 2^53'),
    ('draw_laws.py', '_MANTISSA_BITS'): ('definitional', frozenset({53}), 'the float64 significand is 53 bits'),
    ...
    ('annotation_design.py', '_SPLINE_DEGREE'): ('math', frozenset({3}), "the cubic spline: the lowest degree whose integral of f''^2 is finite with f'' continuous, and whose roughness penalty's null space is exactly the linear functions (Wahba 1990, section 1.3)"),
    ...
    ('scale_mixture_ep.py', '_HELD_ARRAYS_PER_ROW_SET'): ('definitional', frozenset({10}), 'the p x K arrays one held _KernelRows set keeps: its six forms and the four derivatives once formed'),
    ...
    ('scale_mixture_ep.py', '_KRONROD_WEIGHTS'): ('spec', frozenset({0.022935322010529224, 0.06309209262997856, 0.10479001032225019, 0.14065325971552592, 0.1690047266392679, 0.19035057806478542, 0.20443294007529889, 0.20948214108472782}), 'QUADPACK dqk15i (Piessens, de Doncker-Kapenga, Ueberhuber and Kahaner 1983): the 15-point Kronrod rule and its embedded 7-point Gauss rule'),
    ('scale_mixture_ep.py', '_GAUSS_WEIGHTS'): ('spec', frozenset({0.1294849661688697, 0.27970539148927664, 0.3818300505051189, 0.4179591836734694}), 'QUADPACK dqk15i (Piessens, de Doncker-Kapenga, Ueberhuber and Kahaner 1983): the 15-point Kronrod rule and its embedded 7-point Gauss rule'),
    ('scale_mixture_ep.py', '_QUADPACK_ERROR_SCALE'): ('spec', frozenset({200.0}), "QUADPACK's local error estimate: err = asc min(1, (200 |K - G| / asc)^1.5) (dqk15i)"),
    ('scale_mixture_ep.py', '_QUADPACK_ERROR_POWER'): ('spec', frozenset({1.5}), "QUADPACK's local error estimate: err = asc min(1, (200 |K - G| / asc)^1.5) (dqk15i)"),
    ...
    ('scale_mixture_ep.py', '_stationarity'): ('math', frozenset({4.0}), 'the quadratic formula for the slopes error scale t that meets the budget: gamma t^2 + beta t - room = 0, t = 2 room / (beta + sqrt(beta^2 + 4 gamma room))'),
    ('scale_mixture_ep.py', 'fit_hyperparameters.segment_reach'): ('math', frozenset({4.0}), 'the maximum of the quadratic model s f + c f^2 with c < 0 is s^2 / (4 |c|), at f = s / (2 |c|)'),
    ('scale_mixture_ep.py', '_path_gain'): ('math', frozenset({12.0}), "the trapezoid rule's Euler-Maclaurin end correction: (g_start + g_end)'s / 2 + s'(B_end - B_start) s / 12, exact for a quartic along s"),
    ('scale_mixture_ep.py', '_correction_slopes'): ('derived', frozenset({3.0, 6.0, 4.0, 0.25}), 'central and one-sided differences with each value resolved to e: truncation h^2 s / 6 (central) or h s / 2 (one-sided) against e / h, least at h = (3 e / s)^(1/3), error (3^(2/3) / 2) s^(1/3) e^(2/3); the h / 2 difference errs by (h / 2)^2 s / 6 + 2 e / h, or h s / 4 + 4 e / h one-sided'),
    ...
    ('scale_mixture_ep.py', '_KernelRows.derivatives'): ('derived', frozenset({6.0, 3.0, 9.0, -3.0}), 'the recursion A_(n+1) = r A_n - r(1 - r) A_n\', B_(n+1) = -r(1 - r) B_n\' for d^n log Z_k / d eta^n (_components\' docstring)'),
    ...
    ('scale_mixture_ep.py', '_derivative_rows'): ('math', frozenset({3.0}), 'd Var / dh of a normal mixture: E[(mu - m)^3] + 3 E[c (mu - m)] (third central moment of a Gaussian mixture)'),
    ...
    ('full_data_fit.py', '_FullDataFixedPoints._solve'): ('math', frozenset({0.25}), "KL(q || q') to second order in a site change: 1/2 r'Sigma r + 1/4 dtau'(Sigma o Sigma) dtau, the Fisher metric of q's statistics (beta, -beta^2/2); the variance part's 1/4 = 1/2 x 1/2"),
    ...
    ('scale_sampler.py', '_GOLDEN'): ('spec', frozenset({0x9E3779B97F4A7C15}), 'splitmix64 (Steele, Lea and Flood 2014, OOPSLA): the golden-ratio increment, its two multipliers and three shifts'),
    ('scale_sampler.py', '_MIX_FIRST'): ('spec', frozenset({0xBF58476D1CE4E5B9}), 'splitmix64 (Steele, Lea and Flood 2014, OOPSLA): the golden-ratio increment, its two multipliers and three shifts'),
    ('scale_sampler.py', '_MIX_SECOND'): ('spec', frozenset({0x94D049BB133111EB}), 'splitmix64 (Steele, Lea and Flood 2014, OOPSLA): the golden-ratio increment, its two multipliers and three shifts'),
    ('scale_sampler.py', '_SHIFT_FIRST'): ('spec', frozenset({30}), 'splitmix64 (Steele, Lea and Flood 2014, OOPSLA): the golden-ratio increment, its two multipliers and three shifts'),
    ('scale_sampler.py', '_SHIFT_SECOND'): ('spec', frozenset({27}), 'splitmix64 (Steele, Lea and Flood 2014, OOPSLA): the golden-ratio increment, its two multipliers and three shifts'),
    ('scale_sampler.py', '_SHIFT_THIRD'): ('spec', frozenset({31}), 'splitmix64 (Steele, Lea and Flood 2014, OOPSLA): the golden-ratio increment, its two multipliers and three shifts'),
    ('scale_sampler.py', '_UNIFORMS_PER_UPDATE'): ('definitional', frozenset({3}), "the uniforms one member's update draws: its node, the exchange's coin and the exchange's acceptance"),
    ('scale_sampler.py', '_scan'):('math', frozenset({3.0, 6.0}), 'the fourth moment of a Gaussian component: E[(m + sqrt(c) Z)^4] = m^4 + 6 m^2 c + 3 c^2'),
    ('scale_sampler.py', 'observed_information'): ('math', frozenset({0.25}), "Louis' identity for the scale coefficient: Var(beta^2 / (2 v)) = (E[beta^4 / v^2] - A^2) / 4"),
    ('scale_sampler.py', 'empirical_bayes'): ('math', frozenset({0.125}), "the pooled score's Monte Carlo part: (g_A + g_B)/2 has error (e_A + e_B)/2, whose metric square has mean E[(g_A - g_B)' W (g_A - g_B)] / 4, and a decrement is half of it"),
