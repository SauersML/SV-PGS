...
REGISTRY: dict[tuple[str, str], tuple[str, frozenset[object], str]] = {
    ...
    ('dual_solve.py', 'spike_deflation'): ('definitional', frozenset({3}), 'axis indices of a batched matrix transpose'),
    ...
    ('synthetic_store.py', 'COHORT_GROUPS'): ('scenario', frozenset({0.52, 0.22, 0.8, 0.2, 0.18, 0.35, 0.15, 0.05, 0.03}), 'simulator scenario parameters, with the units, code ranges and RNG stream labels they use (synthetic_store module docstring)'),
    ...
    ('synthetic_store.py', 'READ_EVIDENCE_FRACTION'): ('scenario', frozenset({0.18, 0.1}), 'simulator scenario parameters, with the units, code ranges and RNG stream labels they use (synthetic_store module docstring)'),
    ...
    ('synthetic_store.py', 'UNINFORMED_SHARE'): ('scenario', frozenset({0.9, 0.4, 0.45}), 'simulator scenario parameters, with the units, code ranges and RNG stream labels they use (synthetic_store module docstring)'),
    ('synthetic_store.py', 'SINGLE_PATH_R2'): ('scenario', frozenset({0.988, 0.973, 0.89, 0.51}), 'simulator scenario parameters, with the units, code ranges and RNG stream labels they use (synthetic_store module docstring)'),
    ('synthetic_store.py', 'MAF_BIN_EDGES'): ('scenario', frozenset({0.001, 0.005, 0.01, 0.05, 0.1}), 'simulator scenario parameters, with the units, code ranges and RNG stream labels they use (synthetic_store module docstring)'),
    ('synthetic_store.py', 'MAF_R2_MULTIPLIER'): ('scenario', frozenset({0.47, 0.77, 0.85, 0.905, 0.95, 0.52, 0.3, 0.6}), 'simulator scenario parameters, with the units, code ranges and RNG stream labels they use (synthetic_store module docstring)'),
    ('synthetic_store.py', 'PIPELINE_R2_LOSS'): ('scenario', frozenset({0.005, 0.02, 0.03}), 'simulator scenario parameters, with the units, code ranges and RNG stream labels they use (synthetic_store module docstring)'),
    ...
    ('synthetic_store.py', 'STRATA_COMPLEXITY_COUNTS'): ('scenario', frozenset({70223727, 2813198, 91126, 113683, 4193612, 10331238, 453103, 16931, 19895, 707675, 3151800, 139705, 5674, 6140, 219047, 4857251, 225752, 9407, 9894, 343874, 1462958, 71967, 2951, 2906, 103652, 4747491, 237151, 8809, 8855, 301276, 5291687, 3702265, 731046, 652663, 1746407, 820764, 1022386, 327397, 304396, 696724, 252242, 348353, 149748, 136520, 255632, 386030, 588687, 316783, 294037, 425493, 114374, 190850, 109184, 99322, 109204, 355057, 607497, 236426, 142282, 170373, 84392, 67889, 40355, 46958, 667344, 12614, 13069, 8268, 11159, 135069, 3860, 3972, 2606, 3715, 41639, 6119, 6195, 4001, 5834, 56882, 1623, 1387, 878, 1249, 8797, 5602, 4619, 2611, 3187, 20822}), 'simulator scenario parameters, with the units, code ranges and RNG stream labels they use (synthetic_store module docstring)'),
...
PENDING: dict[tuple[str, str], tuple[str, frozenset[object], str]] = {
    ...
    ('all_of_us.py', 'MEASUREMENT_DEFINITIONS.plausible_range'): ('phenotypes', frozenset({120.0, 230.0, 12.0, 90.0, 60.0, 270.0, 25.0, 220.0, 50.0, 150.0, 10.0, 1500.0, 100.0, 0.05, 30.0, 0.2, 20.0, 400.0, 3.0}), 'per-occasion measurement model with learned time-since-event smooths and a contamination component (PHENOTYPES.md)'),
    ...
    ('all_of_us.py', 'LAB_ANALYTE_DEFINITIONS.plausible_range'): ('phenotypes', frozenset({0.1, 30000.0}), 'per-occasion measurement model with learned time-since-event smooths and a contamination component (PHENOTYPES.md)'),
    ...
    ('scale_mixture_ep.py', 'kernel_floor'): ('e2e', frozenset({0.25, 4.0}), 'classify: derive, cite as math, or remove (trust-region ratios 0.25/0.75, lattice minimum 5, chunk bytes, kernel and tail constants)'),
    ('scale_mixture_ep.py', 'tail_mass'): ('e2e', frozenset({-2.0}), 'classify: derive, cite as math, or remove (trust-region ratios 0.25/0.75, lattice minimum 5, chunk bytes, kernel and tail constants)'),
    ('scale_mixture_ep.py', '_legendre_functionals'): ('e2e', frozenset({3}), 'classify: derive, cite as math, or remove (trust-region ratios 0.25/0.75, lattice minimum 5, chunk bytes, kernel and tail constants)'),
    ('scale_mixture_ep.py', 'scale_mixture_prior'): ('e2e', frozenset({5}), 'classify: derive, cite as math, or remove (trust-region ratios 0.25/0.75, lattice minimum 5, chunk bytes, kernel and tail constants)'),
    ('scale_mixture_ep.py', '_row_chunks'): ('e2e', frozenset({16, 8}), 'classify: derive, cite as math, or remove (trust-region ratios 0.25/0.75, lattice minimum 5, chunk bytes, kernel and tail constants)'),
    ('scale_mixture_ep.py', '_components.third'): ('e2e', frozenset({6.0}), 'classify: derive, cite as math, or remove (trust-region ratios 0.25/0.75, lattice minimum 5, chunk bytes, kernel and tail constants)'),
    ('scale_mixture_ep.py', 'quadrature_majorant_ratio'): ('e2e', frozenset({0.25}), 'classify: derive, cite as math, or remove (trust-region ratios 0.25/0.75, lattice minimum 5, chunk bytes, kernel and tail constants)'),
    ('scale_mixture_ep.py', '_maximize_coefficients'): ('e2e', frozenset({0.25, 0.75}), 'classify: derive, cite as math, or remove (trust-region ratios 0.25/0.75, lattice minimum 5, chunk bytes, kernel and tail constants)'),
    ('scale_mixture_ep.py', '_ascend_evidence'): ('e2e', frozenset({0.25, 0.75}), 'classify: derive, cite as math, or remove (trust-region ratios 0.25/0.75, lattice minimum 5, chunk bytes, kernel and tail constants)'),
