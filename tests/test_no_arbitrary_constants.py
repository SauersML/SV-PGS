...
REGISTRY: dict[tuple[str, str], tuple[str, frozenset[object], str]] = {
    ...
    ('scale_mixture_ep.py', '_HELD_ARRAYS_PER_ROW_SET'): ('definitional', frozenset({11}), 'the p x K arrays one held _KernelRows set keeps: its seven forms and the four derivatives once formed'),
    ...
    ('mean_field.py', 'MeanFieldFixedPoints._central_moments'): ('math', frozenset({3.0, 4, 6.0}), 'central moments of a Gaussian mixture from its components: E[(d + sqrt(c) Z)^3] = d^3 + 3 d c, E[(d + sqrt(c) Z)^4] = d^4 + 6 d^2 c + 3 c^2'),
