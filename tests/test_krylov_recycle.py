...
def test_the_local_response_inverts_each_block_of_its_local_matrix(monkeypatch) -> None:
    ...
    monkeypatch.setattr(krylov, "prepare_windows", lambda solve, grams: None)
    monkeypatch.setattr(krylov, "block_covariance", lambda solve, grams, block, array_module, prepared: covariance[np.ix_(blocks[block], blocks[block])])
