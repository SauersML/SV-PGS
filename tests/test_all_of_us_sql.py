...
class _Cdr:
    ...
    def measurement_rows(self, definition: MeasurementDefinition) -> dict[str, dict[str, object]]:
        return self._run(build_all_of_us_measurement_sql(), build_all_of_us_measurement_query_parameters(definition))
...
def _hypertension_cohort(cdr: _Cdr) -> None:
    """Seven people observed from 2010: cases by two dates or one date plus a
    drug, a one-date person, an excluded and a medicated non-case, a control,
    and a case with too little early EHR."""
    early_dates = ("2010-02-01", "2010-03-01", "2010-04-01", "2010-05-01", "2010-06-01")
    for person_id, year_of_birth in ((1, 1950), (2, 1955), (3, 1960), (4, 1965), (5, 1970), (6, 1975), (7, 1980)):
        cdr.person(person_id, year_of_birth, FEMALE if person_id % 2 else MALE)
        cdr.observation_period(person_id, "2010-01-01", "2024-01-01")
        if person_id != 7:
            for date in early_dates:
                cdr.condition(person_id, PAIN, date)
    cdr.condition(1, 320128, "2015-01-01")
    cdr.condition(1, 320128, "2015-01-01")
    cdr.condition(1, 320128, "2016-01-01")
    cdr.condition(2, 320128, "2016-01-01")
    cdr.drug(2, 1307046, "2016-02-01")
    cdr.condition(3, 320128, "2017-01-01")
    cdr.condition(4, 316866, "2018-01-01")
    cdr.drug(5, 1307046, "2019-01-01")
    cdr.condition(7, 320128, "2015-01-01")
    cdr.condition(7, 320128, "2016-01-01")


def test_disease_query_counts_dates_flags_exclusions_and_measures_ehr_depth(cdr: _Cdr):
    hypertension = resolve_disease_definition("hypertension")
    _hypertension_cohort(cdr)
    rows = cdr.disease_rows(hypertension)
    assert rows["1"]["phenotype_occurrence_count"] == 2
    assert rows["1"]["age_at_first_condition"] == pytest.approx(_age("2015-01-01", 1950))
    assert rows["1"]["age_at_observation_end"] == pytest.approx(_age("2024-01-01", 1950))
    assert rows["1"]["pre_landmark_condition_dates"] == 5
    assert rows["2"]["phenotype_occurrence_count"] == 1
    assert rows["2"]["has_disease_medication"] is True
    assert rows["4"]["has_control_exclusion_code"] is True
    assert rows["4"]["phenotype_occurrence_count"] == 0
    assert rows["5"]["has_disease_medication"] is True
    assert rows["6"]["phenotype_occurrence_count"] == 0
    assert rows["6"]["has_control_exclusion_code"] is False
    assert rows["6"]["sex_at_birth_name"] == "male"
    assert rows["7"]["pre_landmark_condition_dates"] == 0

    training_rows, _columns, counts = _prepare_training_rows(hypertension, list(rows.values()))
    targets = {row["person_id"]: row["target"] for row in training_rows}
    assert targets == {"1": 1, "2": 1, "6": 0}
    assert counts == {
        "n_cases": 2,
        "n_controls": 1,
        "n_excluded_sparse_ehr": 1,
        "n_excluded_ambiguous_code": 0,
        "n_excluded_one_date": 1,
        "n_excluded_control_exclusion": 2,
    }


def test_type1_diabetes_is_ambiguous_for_type2_cases_and_controls(cdr: _Cdr):
    type2_diabetes = resolve_disease_definition("t2d")
    for person_id in (1, 2, 3):
        cdr.person(person_id, 1960, FEMALE)
        cdr.observation_period(person_id, "2010-01-01", "2024-01-01")
        for month in range(1, 7):
            cdr.condition(person_id, PAIN, f"2010-0{month}-15")
    for person_id in (1, 2):
        cdr.condition(person_id, 201826, "2015-01-01")
        cdr.condition(person_id, 201826, "2016-01-01")
    cdr.condition(2, 201254, "2017-01-01")
    cdr.condition(3, 4024659, "2012-01-01")
    rows = cdr.disease_rows(type2_diabetes)
    training_rows, _columns, counts = _prepare_training_rows(type2_diabetes, list(rows.values()))
    assert [row["person_id"] for row in training_rows] == ["1"]
    assert counts["n_excluded_ambiguous_code"] == 1
    assert counts["n_excluded_control_exclusion"] == 1
