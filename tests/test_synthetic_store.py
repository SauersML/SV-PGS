...
from sv_pgs.dosage_store import VARIANT_CLASSES, DosageStore, code_sums, dosage_array_directory
...
def test_generated_store_is_deterministic_across_worker_counts_and_matches_its_sidecar(tmp_path: Path) -> None:
    ...
    with DosageStore.open(tmp_path / "parallel") as store:
        ...
        sums, squares = code_sums(store.read_codes(0, store.n_variants))
        assert np.array_equal(sums, store.variant_table.sum_code) and np.array_equal(squares, store.variant_table.sum_code2)
