...
| store | Decoding GT:DS:GP: one parse per genotype, CPU-bound and independent per batch file; then one zstd write per genotype | n2d-standard-224 spot (n2d 5,000 / preemptible 10,000 CPUs), a persistent run disk | Decode cost = n·p entries ÷ (cores × per-core rate). The per-core rate is measured on synthetic batches (below). Workers = CPU threads, each with its share of host memory |
