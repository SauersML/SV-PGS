...
**At genome scale: WITHDRAWN (2026-09-19).** The genome-scale outer spectrum first quoted here (the pencil (B + S, A + S) with λ ∈ [0.89, 5.41], ρ = max|1 − λ| up to 4.41, and "plain EP-EM diverges in 7 of 16") came from speed-floor's measurement, which linearized at the true prior on a non-invariant subspace. The true prior is a saddle of that problem, not a fixed point (compute_floor.md §10.1–10.3). Those numbers are withdrawn.
- **What still stands:** plain EP-EM's fixed-cavity M-step is ill-posed wherever A + S is indefinite, which it was at the true prior on real chr22 LD [semi-real]. **So the outer step uses B,** via b_products.md's closed form, globalized with a trust region. The production outer rate is unmeasured; it will be measured on the engine at chr22 scale with learned λ.

**Block-variance cavities** [machinery, semi-real, speed-floor, `cut_coupling3.json`]:
- Block-local marginal variances give cavity-precision errors with p99 of 54% at block cap 1024 and 25% at cap 4096 on bench-sim's v7 cohort (earlier cohort: 28–58%). The second-order cross-block term brings cap 4096 to p99 1.5%.
