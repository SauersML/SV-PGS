...
- **Hyper step:**
  - the coefficients maximize log Z_EP − ½xᵀS_λx by Newton with the total curvature B = −∇² log Z_EP, with EP re-solved (docs/design/math/ep_eb.md §1.4). The outer loop (`fit_hyperparameters`) sets the weights at each certified EP fixed point, then moves x by (B + S)⁻¹g, globalized by Deuflhard's natural monotonicity test (g'|B + S|⁻¹g at the trial's fixed point must fall, else the step halves). It is never plain EP-EM (x to the fixed-cavity maximizer): that step solves with A + S, maps the error by I − (A + S)⁻¹(B + S), and diverges where the pencil exceeds 2. At production the pencil spans [0.89, 5.4], and plain EP-EM diverged in 7 of 16 rows [semi-real: speed-floor, bench-sim chr22 real-haplotype LD, simulated effects] (lead ruling, 2026-09-19);
...
- **The production engine** (`sv_pgs/scale_mixture_ep.py`) holds everything that involves the prior, for Stage 2, which supplies q's means, marginal variances and linear responses:
  ...
  - The final stationarity check: one central difference of V per interior weight, with its step derived from V's rounding and curvature scale, h = (3ε_V/s)^{1/3}, s = ½(edf + λ‖Rx‖²); the step and its error bound h²s/6 + ε_V/h are recorded. While the gain a Newton step on that difference predicts exceeds the tolerance, the search continues along it.
  - Basins are compared by the Tierney–Kadane-corrected V: the O(1) term κ4/8 + 5κ3²/24 per standardized Schur direction, from exact third and fourth derivatives, replaced by the exact QUADPACK line integral wherever the term exceeds the tolerance.
...
- **No Stage 1** (lead ruling, 2026-09-19). The production outer map has no slow direction (λ_min 0.89–1.00), and the certified Newton-B loop reaches the certificate in 0.7–2.6 outer steps (at most 12 at the worst 95% upper bound) [semi-real: speed-floor, bench-sim chr22 real-haplotype LD, simulated effects]. So an LD-space warm start has nothing to shorten. Caveat: the measured windows reached LD score 19, against a chromosome mean of about 45; speed-floor is re-measuring in high-LD regions.
