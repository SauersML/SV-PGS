...
- **Covariate handling:** they are projected out exactly (Frisch–Waugh–Lovell) before any LD summary is formed. Block-diagonal LD with ancestry structure left in overfit 400× in simulation.
- **D_j, the stored genotype column:** the dosage after two corrections, standardized by its empirical SD in the training fold (never √(2pq), which up-weights ancestry-differentiated variants by (1+F)):
  ...
  - where a validated curve exists, a per-stratum, per-ancestry linear recalibration D* = μ + κ(DS − μ). The recalibration's shape added ≤ 0.012 r², so only the scale is kept.
- **A tandem-repeat (TR) locus** is one signed-length column Z_i = Σ_a Δlen_a · D_ia, summed over every record in the locus, including indels under 50 bp.
  ...
  - Measured: +4–12% R² where TRs carry signal, ≤ 0.8% cost where they don't. Per-allele TR columns were neutral or negative.
- **Non-TR multi-allelic bubbles** keep allele columns, with the REF state given its own column (exchangeable allele states), so predictions don't depend on which allele GRCh38 carries. This is accuracy-neutral and removes the reference dependence.

## 2. Measurement model

- **The column is a measurement of the true genotype G_j.** Imputed SV and TR dosages behave like confident posterior draws (κ ≈ √r² per stratum), not calibrated posterior means. So INFO and dosage-based r² cannot rank SV reliability.
- **Reliability:** r²_j = corr²(D_j, G_j). It is truth-calibrated and triad-corrected: r(D,T1)·r(D,T2)/r(T1,T2) over two independent long-read truths. A per-record model predicts it from site features, and the model ships as a coefficient table (`sv_pgs/imputation_reliability.py`).
  ...
  - The r² estimate never enters the Gram. Adding E[x²] corrections shrinks every effect by r² (measured −0.2 to −3.2% R²).
- **GATK-SV short-read calls are a second measurement B of the same G:** B = α_B + ρ_B·G + e_B. α_B is the false-positive intercept (class precision 0.90–0.97).
  ...
  - **Where GATK-SV per-evidence read-depth genotypes exist** (DEL/DUP ≥ ~1 kb), the three-source identity r²_A = C_AB·C_AC/(C_BC·V_A) replaces the anchor, once validated. Paired-end and split-read genotypes equal the final call (r = 1.0), so they cannot be a third source.
  ...
  - **Acceptance (met in simulation):** gap to the oracle ≤ 0.001 with known r²_A, and ≤ 0.01 (MCAR) / ≤ 0.025 (MNAR) with estimated r²_A. The old closure's VNTR gap was −0.34.
- **The ~12k long-read panel members are stacked as an extra store half** of hard calls on the same sites, with their own cohort covariate. Measured: +7–8% R² at 100k, +18–19% at 50k. Truth rows performed like imputed rows for the same people. Folds are kinship-grouped across the union of both halves.
...
- **Effect prior:** β_j | class c ~ ∫ N(0, u_j · s) g_c(s) ds.
  ...
  - TPB and BayesR-like shapes are special cases. A learned mixing density exists because a fixed-shape TPB lost to BayesR by 0.025–0.10 R² in fifteen Gibbs scenarios.
- **Scale model:** log u_j = level_c + log r̂²_j + d_jᵀθ.
  - The r² coefficient is exactly 1 by derivation: the prior on the true-genotype effect maps to the observed column through r². Free EB could not identify it.
  - level_c and θ get hierarchical EB priors whose mean and variance are learned across classes and across traits (`da928bd`). Cross-trait pooling matters: a single trait's EB recovered only 1–9% of the true-prior gain, against 25–115% for the true prior at n = 10–20k.
...
- **Method:** type-II maximum likelihood under an expectation-propagation approximation, with exact one-dimensional tilted moments. It matched exact Gibbs within ±1.5%. The old GIG mean-field fit was 23–36% worse, and its plug-in fixed point 28–42% worse.
- **Hyper step:**
  ...
  - Plain EM converges at rate ≥ 1 − edf/p, about 0.99 at production scale. From the defaults its reported SV/SNV enrichment was 1.65 whatever the truth.
...
- **Binary traits:** logistic EP with an exact 32-point Gauss–Hermite predictive. Probit was rejected: VB-probit lost 0.012–0.020 AUC, and EP-probit only tied logistic.
...
- **Stage 2: full-data certification** (`exact_polish.py`).
  ...
  - Posterior draws.
  - This stage carries the real weight: a block-diagonal Stage 1 alone was 2.7× off at p/n = 20.
- **Scoring** (`fast_scoring.py`): every trait × fold model and its posterior draws in one read of the store. It is exact to 1e-13; an H100 does 100k × 17.3M in about 100 s.

## 6. Built but not wired
- `pleiotropy_layer.py`, the joint multi-trait EP-EB layer: continuous learned multiplier density, class rates by type-II ML. It gets wired in when Stage 1 lands, then its gain is re-measured. It measured +1.4% on a weaker base, and +7.3% under high trait overlap.

## 7. Measured and rejected (do not re-propose without new evidence)
- A multi-ancestry deviation prior β_{j,a} = β_j + δ_{j,a}: −7 to −46% non-EUR accuracy.
- Refining SV dosages from information already in the imputed files (locus kernels, tag-SNV shrinkage, boosted trees, monotone recalibration for SVs): no PGS gain.
- Sibling-allele coupling (GR-2) beyond the locus model: no gain.
- Fine-structure signal beyond PCs carried by SVs: ΔR² −0.00002 ± 0.00003.
- Per-class free tails estimated from each class's own columns: they cost more than they returned.
- Sparse arithmetic kernels for rare columns in Stage 2: they lose to dense tensor cores at ~1,800 right-hand sides. Rare variants are sparse in storage only.
- gamfit for the r² calibration model: it failed REML certification, and only tied the linear model.
