...
| [math/prior_terms.md](math/prior_terms.md) | The scale model's terms as code (sv_pgs/prior_terms.py, sv_pgs/gene_dosage.py): the derived offset (recalibrated column only), the frequency smooth whose infinite-weight limit is the H^(1+S) power law, the pooled SV-context kernel with its exact roughness, stacking to what the engine can identify, the gene-dosage term (Theorem 5's unit column for cis; a switchable dosage-sensitivity smooth for organismal traits), and the shape score directions |
