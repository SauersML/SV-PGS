...
- **The measurement model** (`sv_pgs/phenotype_measurement.py`; novel-pheno.md §3): h_λ(y_ij) = d_ij'γ + T_i + e_ij, with T_i ~ N(0, τ²) the person's long-run level.
  ...
  - e_ij follows a learned continuous scale mixture: the engine's mixing density over log variance, with a third-difference roughness penalty whose weight maximizes the evidence. Its lattice starts at the variance of the recording resolution, (δ·y^{λ−1})²/12, the smallest resolvable noise.
  - d_ij holds an intercept, age and age² at the occasion, sex at birth, and age × female.
  - The fit is exact in T_i: EM whose E-step integrates each person's level by a certified trapezoid rule (mixing_density.md §11), and the evidence's curvature comes from Louis' identity.
