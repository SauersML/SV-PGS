...
TR-class records (class 2, bench-sim's indels in a repeat, the records its TR truths are drawn from) with a length
change; every other record keeps its own column. The column is
...
from benchmarks.bench_sim.submissions.svpgs_full import TR_CLASS
...
def locus_columns(variants: dict) -> Derived:
    ...
    members = np.flatnonzero((locus >= 0) & (change != 0.0) & (np.asarray(variants["cls"]) == TR_CLASS))
