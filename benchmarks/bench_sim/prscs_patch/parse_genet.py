...
def parse_ldblk(ldblk_dir, sst_dict, chrom):
    # PRSCS_LD_CACHE (baselines-genome addition): a directory holding this function's exact float64 output, saved by
    # np.save on the first call and loaded on a rerun of the same inputs, so a resubmitted fit skips the per-block SVDs
    cache = os.environ.get('PRSCS_LD_CACHE')
    if cache and os.path.exists(os.path.join(cache, 'blk_size.npy')):
        print('... load parsed reference LD on chromosome %d from %s ...' % (chrom, cache))
    ...
    if cache:
        ...
        for blk, matrix in enumerate(ld_blk):
            np.save(os.path.join(cache, 'blk_%d.npy' % blk), matrix)
        np.save(os.path.join(cache, 'blk_size.tmp.npy'), np.array(blk_size))
        os.replace(os.path.join(cache, 'blk_size.tmp.npy'), os.path.join(cache, 'blk_size.npy'))
    return ld_blk, blk_size


def _parse_ldblk(ldblk_dir, sst_dict, chrom):
