...
def fit_expression_batch(trains: Sequence[Any]) -> list[BenchRealPredictor]:
    ...
    return [
        BenchRealPredictor(scoring=scoring, columns=scoring.store_rows, centering="training", input_columns=gene.codes.shape[1], codes_per_unit=units)
        for scoring, gene, units in zip(fitted.scoring, genes, encodings)
    ]
